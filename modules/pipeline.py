...
class VideoPipeline:
    ...
    def setup_video_io(self):
        ...
        for processor in self.processors:
            processor_dir = os.path.join(self.output_dir, processor.name)
            os.makedirs(processor_dir, exist_ok=True)

            output_specs = processor.get_output_specs()
            for output_name, spec in output_specs.items():
                if spec["type"] == "video":
                    writer_path = os.path.join(processor_dir, f"{output_name}.mp4")
                    writer = cv2.VideoWriter(
                        writer_path, fourcc, self.fps, self.frame_size
                    )
            ...
            while ret:
                ...
                for processor in self.processors:
                    try:
                        ...
                        imshow_frames, video_frames = processor.visualize(
                            frame, results
                        )
                        ...
                        print(f"Error processing frame with {processor.name}: {e}")
